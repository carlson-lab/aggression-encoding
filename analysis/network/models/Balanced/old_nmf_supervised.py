...
class NMF_logistic(NMF_base):
	...
	def fit_transform(self,X_tot,X_sup,Y):
		...
		Xi_train,Xi_test, = train_test_split(X,test_size=.2,random_state=42)
		...
		for t in trange(self.nIter):
			...
			# Estimate the gradients and apply them with optimizer
			grad_s = tape.gradient(loss_sup,trainable_variables_s)
			optimizer_s.apply_gradients(zip(grad_s,trainable_variables_s))
