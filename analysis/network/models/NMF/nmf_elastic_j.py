...
class NMF_logistic(NMF_base):
	...
	def fit_transform(self,X,Y):
		...
		A_enc_p = tf.Variable(A_enci[:,0].reshape(-1,1))
		A_enc_n = tf.Variable(A_enci[:,1].reshape(-1,1))
		A_enc_g = tf.Variable(A_enci[:,2:])
		B_enc = tf.Variable(B_enci)

		#####################
		# Get the optimizer #
		#####################
		trainable_variables_init = [A_enc_g,A_enc_p,A_enc_n,B_enc]
		...
		while cont:
			...
			with tf.GradientTape() as tape:
				W_un = tf.nn.softplus(W_r)
				W = tf.math.l2_normalize(W_un,axis=1)
				Ap_a = tf.math.abs(A_enc_p)
				An_a = -1.0*tf.math.abs(A_enc_n)
				A_enc = tf.concat((Ap_a,An_a,A_enc_g),axis=1)

				S_latent = tf.matmul(X_batch,A_enc) + B_enc
				S_ = tf.matmul(X_batch,A_enc) + B_enc
		...
		Phi1_ = tf.Variable(np.atleast_2d(phi_init).T)
		Phi2_ = tf.Variable(np.atleast_2d(phi_init).T)
		B_ = tf.Variable(B_init) 

		trainable_variables = [A_enc_g,A_enc_p,A_enc_n,B_enc,W_r,
								Phi1_,B_,Phi2_]
		...
		for t in trange(self.nIter):
			...
			with tf.GradientTape() as tape:
				...
				#loss_sup = predLoss(Y_batch,Y_pred)
				Y_pred_p = tf.matmul(S_[:,:1],Phi1_) + B_
				Y_pred_n = tf.matmul(S_[:,1:2],Phi2_) 
		...
		self.Phi1 = Phi1_.numpy()
		self.Phi2 = Phi2_.numpy()
		self.B_ = B_.numpy()
