...
class NMF_base(object):
	...
	def _batchX(self,X,wg):
		N = X.shape[0]
		idx = rand.choice(N,size=self.batchSize,replace=False)
		X_batch = X[idx]
		wg_batch = wg[idx]
		return X_batch,wg_batch
...
class NMF_logistic(NMF_base):
	...
	def fit_transform(self,X,Y,weights_s=None,weights_g=None):
		...
		Xi_train,Xi_test,wg_train,wg_test = train_test_split(X,weights_g,
											test_size=.2,random_state=42)
		Xi_train = Xi_train.astype(np.float32)
		Xi_test = Xi_test.astype(np.float32)
		wg_train = wg_train.astype(np.float32)
		wg_test = wg_test.astype(np.float32)
		while cont:
			X_batch,wg_batch = self._batchX(Xi_train,wg_train)
			...
			if jj % 50 == 0:
				...
				S_ = tf.nn.softplus(S_latent)
				a = np.ones(self.batchSize).astype(np.float32)
				print('>>>>>>>>>>>>')
				print(loss.numpy())
				loss = L2_loss(X_batch,X_recon,a)
				print(jj,loss.numpy())
		...
		print(roc_auc_score(Y,S_tr2[:,0]))
		print('>>>>>>>>>>>')

		S_tr = tf.nn.softplus(tf.matmul(X,A_enc) + B_enc)
		S_tr2 = S_tr.numpy()
		mod_lm = lm.LogisticRegression()
		mod_lm.fit(S_tr2,Y)
		print(mod_lm.coef_)
		...
		trainable_variables_sup = [Phi_,B_]
		trainable_variables_decoder = [W_r]
		trainable_variables_enc = [A_enc,B_enc]
		...
		for t in trange(self.nIter):
			...
			grad = tape.gradient(loss_recon,trainable_variables_enc)
			optimizer_e.apply_gradients(zip(grad,trainable_variables_enc))
			...
			grad = tape.gradient(loss_sup,trainable_variables_sup)
			optimizer_s.apply_gradients(zip(grad,trainable_variables_sup))
			...
			with tf.GradientTape() as tape:
				...
				Y_pred = tf.matmul(S_[:,:self.n_blessed],Phi_) + B_
				loss_sup = predLoss(Y_batch,Y_pred,ws_batch)
			grad = tape.gradient(loss_sup,trainable_variables_enc)
			optimizer_e2.apply_gradients(zip(grad,trainable_variables_enc))
			...
			grad = tape.gradient(loss_recon,trainable_variables_decoder)
			optimizer_d.apply_gradients(zip(grad,
											trainable_variables_decoder))
