...
class NMF_logistic(NMF_base):
	...
	def fit_transform(self,X,Y):
		...
		phi_mul = tf.constant(pm.astype(np.float32))
		trainable_variables = [A_enc,B_enc,W_r,Phi_l,B_]
		...
		for t in trange(self.nIter):
			...
			with tf.GradientTape() as tape:
				...
				Phi_a = tf.math.abs(Phi_l)
				Phi_ = tf.multiply(Phi_a,pm)

				AAT  = tf.matmul(tf.transpose(A_enc),A_enc)

				penalty_orth = 20.0*tf.math.abs(AAT[0,1]/tf.sqrt(AAT[0,0]*AAT[1,1]))
