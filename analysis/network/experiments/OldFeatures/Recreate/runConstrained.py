...
from nmf_constrained import NMF_logistic
...
components = myDict['components_']
A_ = myDict['A']
B_ = myDict['B']
Phi = myDict['phi']

A_enc_p = np.atleast_2d(A_[:,0]).T
B_enc_p = np.atleast_2d(B_[0])
components_p = np.atleast_2d(components[0])
Phi_p = Phi

model = NMF_logistic(nFact-1,nIter=nIter,LR=5e-3,mu=.15)
S_train,S_train_p = model.fit_transform(X_train,y_train,A_enc_p,B_enc_p,
					components_p,Phi_p)

S_test,S_test_p = model.transform(X_test)

'''
model = NMF_logistic(nFact,nIter=nIter,LR=5e-3,mu=.15)
S_train = model.fit_transform(X_train,y_train)
S_test = model.transform(X_test)
'''
components_ = model.components_
X_recon_tr = np.dot(S_train,components_) + np.dot(S_train_p,components_p)
X_recon_te = np.dot(S_test,components_) + np.dot(S_test_p,components_p)
...
for i in range(10):
	...
	S_train2 = model.transform(X_train[40000:]) S_train = np.vstack((S_train1,S_train2))
	S_test = model.transform(X_test)
