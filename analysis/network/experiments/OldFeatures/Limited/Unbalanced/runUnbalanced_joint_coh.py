...
S_train,encoder = model.fit_transform(X_train,y_train,weights_s=weights_s,
							weights_g=weights_g)
S_test = model.transform(X_test,encoder)
components_ = model.components_

sname = './Ccheckpoints/Model_enc_' + str(int(100*mu))
...
pname = 'CUnbalanced_Joint_12_enc_' + str(int(100*mu)) + '.p'
