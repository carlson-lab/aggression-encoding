...
from nmf_joint_vae import dNMF_IS_vae
...
training_pdict = {'batch_size':128,'num_iterations':2000}
reg_pdict = {'reg_features':10.0}
model = dNMF_IS_vae(nFact,supervision_strength=1.0,
...
plt.plot(model.losses_supervision)
plt.show()
...
myDict['rTest1_C'] = roc_auc_score(y_test[idx1],y_df_test[idx1])
myDict['rTest2_C'] = roc_auc_score(y_test[idx2],y_df_test[idx2])
myDict['rTest3_C'] = roc_auc_score(y_test[idx3],y_df_test[idx3])
...
myDict['A_enc'] = model.A_
myDict['B_enc'] = model.B_
myDict['lvar_mat'] = model.lvar_mat

pname = 'Joint_VAE_' + str(mu) + '.p'
